# datt1=dat_AA; ind1=19; s=c(5,10,20,30,40); n_fold=5; g=0.01; q=0.0001
OR_AAk_tmle_ctmle <- function(datt1,ind1, s = c(5,10,50,100,200), n_fold=5, g=0.01, q=0.0001){
  # # identify the column for AA_k
  # ind <- which(colnames(datt1)==paste0("W",w_i))
  # levels for certain position
  l <- length(table(datt1[,ind1]))
  # # names for AA in certain position
  # nams <- unique(datt1[,ind1])
  
  # results from cross validation
  cv_top <- cv_triplets(fold=n_fold, datt=datt1, ind=ind1, selections=s, mnds=10, tolg = g, tolQ = q)
  
  # results from full data
  full_top <- AAk_step2(datt = datt1, ind=ind1, mnds=10, ntrees=1000, selections=s, tolg = g, tolQ = q)
