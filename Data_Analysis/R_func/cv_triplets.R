# cross validation
# fold=4;datt=dat_AA;mnds=10;ntrees=1000;levels=4;ind=19;selections=c(5,10,20,100,200);tolg=0.01; tolQ=0.0001 #;top=tops
# output:
...
cv_triplets <- function(fold=4,datt=dat,ind, mnds=10, ntrees=1000,selections,tolg=0.01, tolQ=0.0001){
  ###### funciton1: given Qnstar, get empirical risk
  neg_loglik <- function(Qn_obs, Y){
    # negative log-likelihood ---empirical loss
    loglik1 <- -1/length(Y)*sum(Y*log(Qn_obs)+(1-Y)*log(1-Qn_obs), na.rm = T)
    # loglik1 <- -1/length(Y)*sum(Y*log(Qn_obs)+(1-Y)*log(1-Qn_obs))
    return(loglik1)
  }
  
  ###### funciton2: ic_cov for tops
  ic_cov_func <- function(Qn_v, gn_v, dat_v=validate){
  ...
  empirical_risk_apply <- function(train,validate){
    ...
    for(i in seq(levels)){
      temp = validate
      temp[,ind]=rep(nams[i],nrow(validate))#levels(temp[,ind])=as.factor(seq(levels))
      newdat[[i]] = temp
    }
    rm(temp,i)
    Qn10_validate = list()
    ...
    # truncate Qn10: values <0.0001 will be replaced by tolQ
    Qn10_validate <- rapply(Qn10_validate, f=function(x) ifelse(x==0,tolQ,x), how="replace" )
    
    rm(newdat)
    
    # Qn_obs for validation dataset
    # map back from nams=('C', 'K', 'L') to col_ind=(1,2,3)
    Qn_validate_obs = Reduce(cbind,Qn10_validate)[cbind(1:num,as.numeric(plyr::mapvalues(validate[,ind],nams,seq(nams))))]
    ...
    for(i in seq(selections)){
      ...
      if(i==1){
        ...
        QnStar <- fluctuate_Q(Y = train[,1], A = AA_k, W = datt_k, Qn = Qn10, gn = gn10, a_0 = nams)
        # TMLE use original Qn10 and gn10 to update Qnstar
        QnStar_v_TMLE <- pred_fluctuate_Q(Y = train[,1], A = AA_k, W = datt_k, Qn = Qn10_validate, gn = gn10_validate, a_0 = nams,
                                          alpha = do.call("c", lapply(QnStar, function(x){return(x$eps)})))
      }else{
        # fluctuate the initial Qn based on gn
        QnStar <- fluctuate_Q(Y = train[,1], A = AA_k, W = datt_k, Qn = triplets[[i-1]][[2]], gn = gn10, a_0 = nams)
        QnStar_v_TMLE <- pred_fluctuate_Q(Y = train[,1], A = AA_k, W = datt_k, Qn = Qn10_validate, gn = gn10_validate, a_0 = nams,
                                          alpha = do.call("c", lapply(QnStar, function(x){return(x$eps)})))
      }
      ...
      # map back from nams=('C', 'K', 'L') to col_ind=(1,2,3)
      Qn_obs <- Qn_obs[cbind(1:nrow(train),as.numeric(plyr::mapvalues(AA_k,nams,seq(nams))))]
      ...
      if(loglik1>loglik){
        ...
        QnStar1 <- fluctuate_Q(Y = train[,1], A = AA_k, W = datt_k, Qn = tmp[[3]], gn = gn10, a_0 = nams)
        # obtain the observed Qn to calculate empirical loss
      ...
      Qnstar_validate_obs[[i]] <- plogis(qlogis(Qn_validate_obs) + 1/gn_validate[cbind(seq(num),as.numeric(plyr::mapvalues(validate[,ind],nams,seq(nams))))]*eps[as.numeric(plyr::mapvalues(validate[,ind],nams,seq(nams)))])
      
      #Qnstar_validation (CTMLE) update from Qnstar_validate
      Qnstar_v_CTMLE <- pred_fluctuate_Q(Y = train[,1], A = AA_k, W = datt_k, Qn = Qn10_validate, gn = gn10_validate, a_0 = nams,alpha = eps)
  ...
  # transform back to updated Qnstar for tmle (QnStar_v_TMLEs)
  ...
  # transform back to updated Qnstar for both ctmle (QnStar_v_CTMLEs)
